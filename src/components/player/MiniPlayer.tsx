...
export function MiniPlayer() {
  ...
  const nowPlayingOpen = usePlayer((state) => state.nowPlayingOpen);
  const openNowPlaying = usePlayer((state) => state.openNowPlaying);
  const item = usePlayer((state) => state.queue[state.index]);

  if (!ready || !queue.length || !item || nowPlayingOpen) return null;
